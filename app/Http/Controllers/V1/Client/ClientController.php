    ...
    private function setSubscribeInfoToServers(&$servers, $user, $rejectServerCount = 0)
    {
        ...
        if ($rejectServerCount > 0) {
            array_unshift($servers, array_merge($servers[0], [
                'name' => "Filtered out {$rejectServerCount} routes",
            ]));
        }
        ...
        $expiredDate = $user['expired_at'] ? date('Y-m-d', $user['expired_at']) : __('Never expires');
        $userService = new UserService();
        ...
        if ($resetDay) {
            array_unshift($servers, array_merge($servers[0], [
                'name' => "Days until next reset: {$resetDay} days",
            ]));
        }
        array_unshift($servers, array_merge($servers[0], [
